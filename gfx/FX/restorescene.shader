...
PixelShader =
{
    ...
    [[
        float3 validateColorComponentExtended(float3 inputColor) 
        {
            ...
            float3 colorDirection = normalizedColor * (colorMagnitude * 0.9 + 0.15); // Смягчаем контраст
            ...
            finalColor = lerp(finalColor, inputColor, 0.3); // Смешиваем с оригинальным цветом
            return finalColor * 0.994 - 0.1; // Немного поднимаем минимальную яркость
        }
