...
THRESHOLD_BITRATE = 192_000  # 192 kbps в бит/с
...
def choose_encoder(codec_name: Optional[str]) -> Tuple[str, list[str]]:
    """
    Возвращает имя кодека и список параметров ffmpeg для перекодирования в 192 kbps.
    Сохраняем семейство кодека, если возможно.
    """
    if codec_name == "opus":
        return "libopus", ["-c:a", "libopus", "-b:a", "192k"]
    # По умолчанию — Vorbis в OGG
    return "libvorbis", ["-c:a", "libvorbis", "-b:a", "192k", "-compression_level", "5"]

def transcode_to_192(input_path: Path, dry_run: bool = False, verbose: bool = False) -> bool:
    """
    ...
    if bitrate <= THRESHOLD_BITRATE + EPSILON:
        if verbose:
            print(f"[=] {input_path} — {bitrate/1000:.1f} kbps, <= 192 kbps. Пропуск.")
        return False

    enc_name, enc_args = choose_encoder(codec_name)
    if verbose:
        print(f"[>] {input_path} — {bitrate/1000:.1f} kbps → перекодирование в 192 kbps ({enc_name}).")
...
def main():
    parser = argparse.ArgumentParser(description="Рекодер .ogg > 192 kbps → 192 kbps в папке music")
    ...
    for ogg in music_dir.rglob("*.ogg"):
        total += 1
        try:
            did = transcode_to_192(ogg, dry_run=args.dry_run, verbose=args.verbose)
            if did:
                changed += 1
            else:
                skipped += 1
